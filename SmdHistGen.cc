...
int SmdHistGen::process_event(PHCompositeNode *topNode)
{
    ...
    std::cout << "Found BEGRUNEVENT" << std::endl;
  {
	...
	// exit(1)
	for (int i = 0; i < 120; i++) {spinPatternBlue[i] = 2;}
      }
	...
	// exit(1)
	for (int i = 0; i < 120; i++) {spinPatternYellow[i] = 2;}
      }
    }
  ...
  //fake spin pattern - random +1 or -1 in yellow and blue beam spin patterns, 120 bunches
  /*
  for(int i=0; i < 120; i++){
    spinPatternBlue[i] = rndm.Integer(2)*2 - 1;
  }
  for(int i=0; i < 120;i++){
    spinPatternYellow[i] = rndm.Integer(2)*2 - 1;
    }*/
  /*
  // Printing the generated patterns
  std::cout << "Blue Pattern: ";
  for (int i = 0; i<120; ++i) {
    std::cout << spinPatternBlue[i] << " ";
  }
  std::cout << std::endl;

  std::cout << "Yellow Pattern: ";
  for (int i = 0; i < 120; ++i) {
...
//____________________________________________________________________________..
 ...
 void SmdHistGen::CountLRUD() // compute LR and UD asymmetries
 {
   ...
   //for each bunch number calculate the hit numbers
  ...
  for (int bunchnumber = 0; bunchnumber < NBUNCHES; ++bunchnumber) {
    int blueSpin = spinPatternBlue[bunchnumber];
    float north_x = smd_pos[0];
    float north_y = smd_pos[1];
   int yellowSpin = spinPatternYellow[bunchnumber];
